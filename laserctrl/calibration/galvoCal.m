...
if get(obj.camON,'Value') == true
    stop(obj.vid);
    set(obj.camON,'Value',0);
end
if isempty(obj.vid) || ~isfield(obj,'vid')
    imaqreset;
...
for ii = 1:numFrames
    ...
    if length(STATS)~=1 || MaxIntensityOfFrame(ii)<(.90)*IntensityOfSpot
...
figure(h4), imagesc(dataRead(:,:,:,1)); colormap gray; axis image; set(gca,'XDir','reverse');
