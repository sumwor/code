...
lsrL.DICode             = 0;
lsrL.varlist            = {'DIdata';'DICode';'time'};
lsrL.saveCount          = 1;
lsrL.savestr            = sprintf('%s%s\\%s_laserLog.mat',lsr.savepathroot,lsr.mouseID,lsr.fn);
...
if lsr.preSetOn % for preset experiments (e.g. cfos)
    ...
    switch lsr.manualTrigger
        case true % manual trigger
            ...
            set(obj.statusTxt,'String','Idle','foregroundColor',[.3 .3 .3])
        case false  %controled by Presentation
...
%                     lsrL.presentationState=0;
            ...
            %figure(obj.fig)
            %save the laserlog temp file: lsrL.templog
            savelog(lsrL);
