...
switch command
    ...
    case 'close' % stop DAQ session
        DAQs.ao.stop(); DAQs.ai.stop(); 
        delete(DAQs.ao); delete(DAQs.ai);  
        daqreset;
end
...
% -------------------------------------------------------------------------
%% Load buffer and prepare AO channels  %data output----galvo
...
% -------------------------------------------------------------------------
%% acquire analog input continuously
% function [aidata,DAQs,lsr] = dataInputCont(DAQs,lsr)
% aidata = DAQs.ai.startBackground;
% end
% %% parameter list
% 
...
% laserPort    =   'COM5';             % for serial communication with laser
% nidaqDevice  =   {'Dev1';'Dev2'};    % NI-DAQ device identifier; if more than one use cell array
% aoChannels   =   {[0 1];[0 1]};      % analog output Channels (use cell if mpre than one device)
% aiChannels   =   {[0 1];[]};         % analog input Channels for virmen control (use cell if mpre than one device)
% galvoCh      =   [1 2];              % index of channels controlling galvos (attention for device mounting order!)
% lsrPowerCh   =   3;                  % channel index for modulating laser intensity
% lsrFreqCh    =   4;                  % channel index for modulating laser frequency
% analogVRange =   [-10 10];           % Analog voltage range for NI-DAQ lines
% buffSize     =   3;                  % Buffer size in seconds
% buffRefill   =   .5;                 % Buffer reload size in seconds by event listener
% rate         =   1000;               % DAQ rate in Hz
% duration     =   Inf;                % for property DurationInSeconds, Inf for continuous  