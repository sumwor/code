...
load(sprintf('%s\\grid\\veryReducedGrid.mat',lsr.rootdir),'grid'); % load default grid
lsr.grid        = grid;
lsr.gridLabel   = 'veryReducedGrid.mat';
...
if get(obj.registerIm,'Value') == true
  ...
  if isempty(lsr.currIm)
    ...
    frame = imread(thisfn);
    cd(thisdir)
    lsr.currIm  = obj.camData; %double(frame);
%     obj.camData = frame;
  end
...
%   lsr.refPxl  = transformPointsInverse(lsr.imTform, lsr.refPxl);
%   lsr         = computeOuputData(lsr);
...
if runOnLsr
  warndlg('Run this mouse on laser')
else
  warndlg('Just training for this mouse')
end
...
% set ramp down duartion with input box
...
if strcmpi(lsr.epoch,'3epochs')
  lsr.epoch = {'cueHalf1';'cueHalf2';'mem'};
end
...
% start presentation experiment
function ready_callback(~,event)
global obj lsr

lsr.okFlag = 1;  %set this to 1 temporally to bypass the registration, 9/13/18
...
% cfos (version 1)
function cfospreset_callback(~,event)
global obj lsr
if get(obj.cfospreset,'Value') == true
  load([lsr.rootdir '\grid\cfos.mat'],'grid','locDur','cycleDur','powers','maxDurMin')
  updateConsole('loaded cfos.mat')
  lsr.grid            = grid;
  lsr.gridLabel       = 'cfos.mat';
  lsr.preSetOn        = true;
  lsr.presetLocDur    = locDur; % duration per setim spot in sec
  lsr.presetCycleDur  = cycleDur; % total cycle duration in sec
  lsr.presetMaxDurMin = maxDurMin; % expt duration in min
  lsr.ephys           = false;
  ...
  lsr = computeOuputDataPreSet(lsr);
...
% cfos (version 2, longer inactivation, shorter cycle)
function cfos2preset_callback(~,event)
global obj lsr
if get(obj.cfos2preset,'Value') == true
  load([lsr.rootdir '\grid\cfos2.mat'],'grid','locDur','cycleDur','powers','maxDurMin')
  updateConsole('loaded cfos2.mat')
  lsr.grid            = grid;
  lsr.gridLabel       = 'cfos2.mat';
  lsr.preSetOn        = true;
  lsr.presetLocDur    = locDur; % duration per setim spot in sec
  lsr.presetCycleDur  = cycleDur; % total cycle duration in sec
  lsr.presetMaxDurMin = maxDurMin; % expt duration in min
...
obj.cfospreset = uicontrol   (obj.presetpan,                        ...                              
                        'String',               'C-fos',            ...
                        'Style',                'pushbutton',       ...
                        'Units',                'normalized',       ...
                        'Position',             [.1 .7 .35 .25],    ...
                        'Callback',             @cfospreset_callback,...
                        'foregroundcolor',      'b',                ...
                        'fontsize',             11,                 ...
                        'horizontalAlignment',  'center',           ...
                        'fontweight',           'bold');
                    
obj.cfos2preset = uicontrol   (obj.presetpan,                       ...                              
                        'String',               'C-fos v2',         ...
                        'Style',                'pushbutton',       ...
                        'Units',                'normalized',       ...
                        'Position',             [.55 .7 .35 .25],    ...
                        'Callback',             @cfos2preset_callback,...
                        'foregroundcolor',      'b',                ...
                        'fontsize',             11,                 ...
                        'horizontalAlignment',  'center',           ...
                        'fontweight',           'bold');
